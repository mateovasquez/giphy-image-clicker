...
describe('Loads and displays correctly gifs and selected gif from list', () => {
  ...
  it('Gets gifs data from GIPHY API and shows them in list', () => {
    cy.get('.container__list__option .container__list__option__text').first().should('have.text', 'TEST_TITLE')
  ...
  it('Selects an option from the list and shows gif in the right side', () => {
    cy.get('.container__list__option').first().click()
    ...
    cy.get('.container__detail__image').should('be.visible')
  })

  it('Clicks on the image and increments count', () => {
    cy.get('.container__list__option').first().click()
    cy.get('.container__detail__description').should('include.text', 'Image clicks: 0');
    cy.get('.container__detail__image').click()
    cy.get('.container__detail__description').should('include.text', 'Image clicks: 1');
    cy.get('.container__detail__image').click()
    cy.get('.container__detail__description').should('include.text', 'Image clicks: 2');
  })
  ...
  it('Clicks on different option images and keeps count ', () => {
    ...
    cy.get('.container__detail__image').click()
    cy.get('.container__detail__image').click()
    cy.get('.container__detail__description').should('include.text', 'Image clicks: 3');
    
    // Click second option image with 0 clicks and clicks once
    cy.get('.container__list__option').last().click()
    cy.get('.container__detail__description').should('include.text', 'Image clicks: 0');
    cy.get('.container__detail__image').click()
    cy.get('.container__detail__description').should('include.text', 'Image clicks: 1');
